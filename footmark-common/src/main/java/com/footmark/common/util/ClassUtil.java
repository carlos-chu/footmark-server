...
public abstract class ClassUtil {
    ...
    public static Method getMethodByField(String fieldName, Class<?> clz) {
        ...
        try {
            ...
            logger.warning(String.format("Counld not find method by field: %s", fieldName));
        }
        return method;
    }
    ...
    public static Method getMethod(String name, Class<?> clz, Class<?>... parameterTypes) {
        ...
        try {
            ...
            logger.warning(String.format("Counld not find method: %s from class %s", name, clz.getName()));
            try {
                method = clz.getSuperclass().getDeclaredMethod(name, parameterTypes);
            } catch (NoSuchMethodException e1) {
                logger.warning(String.format("Counld not find method: %s from class %s", name, clz.getSuperclass()
                        .getName()));
            }
        }
        return method;
